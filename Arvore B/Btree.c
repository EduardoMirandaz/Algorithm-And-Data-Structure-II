...
BT_PAGE *createPage(){
  ...
  for (int i = 0; i < MAXKEYS; i++){
      ...
      memset(page->records[i], DUMMY_VALUE, 1);
  }
  memset(page->childs, DUMMY_VALUE, CHILDSIZE);
  page->numberOfKeys = 0;
  page->isLeaf = TRUE;
...
// Recursiva para encontrar o rrn
...
essa função é chamada por _bTreeInsert até achar uma folha para então 
...
/* Retorna o index para inserir um record. */
...
BT_PROMOTED_KEY *_bTreeInsert(BT_PROMOTED_KEY *newRecord, BT_PAGE *page, FILE *filePointer)
{
 ...
 void bTreeInsert(INDEX *newRecord, BT_PAGE *root, FILE *filePointer) {
  ...
  BT_PROMOTED_KEY *newRecordKey = createNewPromotedKey(newRecord, DUMMY_VALUE, DUMMY_VALUE);
...
Errors writePageIntoFile(long rrn, BT_PAGE *page, FILE *filePointer)
{
...
void setLeaf(BT_PAGE **page, boolean isLeaf) {
  (*page)->isLeaf = isLeaf;
}